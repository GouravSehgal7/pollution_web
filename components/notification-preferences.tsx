...
export function NotificationPreferences({ userId }: NotificationPreferencesProps) {
  ...
  // Add new notification time
  const addNotificationTime = () => {
    if (preferences.notificationTimes.length < 5) {
      setPreferences((prev) => ({
        ...prev,
      ...
      toast({
        title: "Maximum times reached",
        description: "You can only set up to 5 notification times per day.",
        variant: "destructive",
      })
    }
  }
  ...
  const removeNotificationTime = (index: number) => {
    if (preferences.notificationTimes.length > 1) {
      const newTimes = [...preferences.notificationTimes]
      ...
      toast({
        title: "Cannot remove",
        description: "You must have at least one notification time.",
        variant: "destructive",
      })
    }
  }
  ...
  const requestPermission = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "You will now receive AQI alerts at your specified times.",
      })
    } catch (error) {
  ...
  const savePreferences = async () => {
    ...
    try {
      ...
      // Sort notification times to ensure they're in chronological order
      const sortedTimes = [...preferences.notificationTimes].sort()
      ...
      if (userPrefsDoc.exists()) {
        await updateDoc(doc(db, "userPreferences", userId), prefsToSave)
      } else {
    ...
    <Card className="bg-white/10 backdrop-blur-sm border-white/20">
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="notification-frequency" className="text-white">
            ...
            <Select
              ...
              disabled={!preferences.enabled || permissionStatus !== "granted"}
            >
              <SelectTrigger id="notification-frequency" className="bg-white/10 border-white/20 text-white">
              ...
              <SelectContent className="bg-black/80 border-white/20 text-white">
                ...
                <SelectItem value="threshold-only">Only When Threshold Crossed</SelectItem>
              </SelectContent>
            </Select>
          </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <Label className="text-white">Notification Times</Label>
              <Button
                variant="outline"
                size="sm"
                onClick={addNotificationTime}
                disabled={
                  !preferences.enabled || permissionStatus !== "granted" || preferences.notificationTimes.length >= 5
                }
